...
```bash
# Everything: both suites, lint and the secret scan
./scripts/test-all.sh

# Or one workspace at a time
npm test --prefix api        # handler, middleware and migration tests
...
CI runs both suites plus lint on every push and pull request, with the
